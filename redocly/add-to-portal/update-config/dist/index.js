...
try {
    ...
    editYaml(referencePage, (content) => {
    ...
    editYaml("./sidebars.yaml", (content) => {
        const page = content.training.find((p) => p.page === referencePage);
        if (!page) {
            content.training.push({ "label": name, "page": referencePage });
        }
        return content;
    });
    editYaml("./siteConfig.yaml", (content) => {
